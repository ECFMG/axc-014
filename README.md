...
| --- | --- |
| `pnpm run dev` | Builds workspace dependencies, then starts the API through portless with `tsc --watch` and `node --watch`. The hostname is `axc.<worktree>.localhost`, where `<worktree>` comes from `WORKTREE_NAME` or the directory name. |
...
| `pnpm run start` | Serves that Rolldown bundle in the documented local runtime (`AXC_LOCAL_RUNTIME=1`, environment `local`) on port 7071 unless `PORT` is set. |
...
Snyk is the local CLI only. The scaffold does not publish snapshot results and does not pass a remote repository URL. The organization passed to Snyk is `agentcourses`. When the CLI is not authenticated, verify prints `Snyk: SKIPPED NON-BLOCKING: credentials unavailable (local Snyk CLI is not authenticated)` and continues. A scan that reports vulnerabilities fails the gate.
...
- `apps/api` composes `@cellix/api-core`, application services, and `@axc/rest`.
...
- `packages/axc-verification/acceptance-api` calls `GET /health` over HTTP against the composed API.
